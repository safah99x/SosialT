...
export function renderEventBringCircle(container, { id, params = new URLSearchParams() }) {
  ...
  const bringGateHost = document.createElement('div');
  bringGateHost.className = 'bring-circle__contacts-gate';
  let selectedCircleId = null;

  function syncBringGate() {
    bringGateHost.innerHTML = '';
    if (selectedCircleId !== 'custom' || !contactsPrimingNeeded()) return;
    bringGateHost.appendChild(buildContactsPrimingCard('bringFriends', (allowed) => {
      if (allowed) {
        const t = document.createElement('div');
        t.className = 'sosialt-toast';
        t.textContent = 'Contacts on — finish your note and send.';
  ...
  step1.appendChild(bringGateHost);
