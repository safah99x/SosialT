  ...
  screen.innerHTML = `
    <header class="chat-header${isPrivate ? ' chat-header--private' : ''}${isDraftLayout ? ' chat-header--tight' : ''}">
      ...
      <div class="chat-header__title">
        ...
        <span>${isPrivate ? 'Circle chat · listing unchanged' : `Event · ${event.group}`}</span>
      </div>
...
function renderMessage(m, event) {
  ...
  if (m.from !== 'self') {
    ...
    from.className = 'chat-line__from';
    from.textContent = PEOPLE[m.from]?.name || m.from;
    const bubble = document.createElement('div');
    bubble.className = 'chat-bubble chat-bubble--them';
    bubble.textContent = m.body;
    stack.appendChild(from);
