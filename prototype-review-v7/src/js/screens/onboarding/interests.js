 ...
 * original tile picker. v7: no global “skip onboarding” control; users
 * advance with Continue only (last step allows continuing without picks).
 *
 * Completed quiz may still trigger the full-screen quiz nudge later
...
export function renderInterestsQuiz(container) {
  ...
  // Footer (Continue only — v7 onboarding).
  const ctaWrap = document.createElement('div');
  ctaWrap.className = 'cta-wrapper onb-interests__cta-wrap';
  const cta = createCTAButton('Continue', () => advance());
