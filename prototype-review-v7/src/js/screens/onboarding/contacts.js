// REVIEW v7: Manual contact add only (QA / Don’t Allow path). No “Skip” — one Continue CTA.
...
function renderManualContacts(container) {
  ...
  screen.innerHTML = `
    ...
    <footer class="onb-contacts-manual__footer">
      <button type="button" class="cta-button onb-contacts-manual__cta" id="mc-continue">Continue</button>
    </footer>
  `;
  ...
  screen.querySelector('#mc-continue').addEventListener('click', () => {
    const nameOk = nameIn.value.trim().length > 0;
    const phoneOk = phoneIn.value.replace(/\D/g, '').length >= 6;
    if (nameOk && phoneOk) {
      setSession({
        contactsAllowed: false,
        manualContactDraft: { name: nameIn.value.trim(), phone: phoneIn.value.trim() },
      });
      toast(screen, 'Saved. Invite them anytime from a plan.');
    } else {
...
export function renderContacts(container, { params } = {}) {
  ...
  screen.querySelector('#ios-deny').addEventListener('click', () => {
    setSession({ contactsAllowed: false, contactsAccessResolved: true });
