...
export function renderCreateEvent(container, { params: routeParams } = {}) {
  ...
  const defaultTitle = publicEvent?.title || 'Rooftop hang · golden hour & playlists';
  ...
  // ── Photo demo: real hero imagery when reviewer taps Add photo (v7). ──
  photoWrap.querySelector('#add-photo-btn').addEventListener('click', () => {
    photoWrap.querySelector('#add-photo-btn').setAttribute('hidden', '');
    const thumb = photoWrap.querySelector('#photo-thumb');
    const img = thumb.querySelector('.photo-thumb__img');
    img.style.backgroundImage = 'url(\'https://images.unsplash.com/photo-1492684223066-81342ee5ff30?auto=format&fit=crop&w=640&q=80\')';
    img.style.backgroundSize = 'cover';
    img.style.backgroundPosition = 'center';
    const icon = img.querySelector('.photo-thumb__icon');
  ...
  photoWrap.querySelector('#photo-remove').addEventListener('click', () => {
    ...
    const img = photoWrap.querySelector('.photo-thumb__img');
