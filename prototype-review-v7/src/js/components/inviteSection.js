 ...
 *
 * v7: First visit to Friends or Not on SosialT shows inline contacts priming (not during onboarding).
 */
...
export function createInviteSection() {
  ...
  gateHost.className = 'invite-section__gate-host';
  wrapper.appendChild(gateHost);

  function syncContactsGate(panelId) {
    gateHost.innerHTML = '';
    if (!contactsPrimingNeeded()) return;
    if (panelId !== 'friends' && panelId !== 'contacts') return;
    const variant = panelId === 'friends' ? 'friendsTab' : 'contactsOffApp';
    const card = buildContactsPrimingCard(variant, (allowed) => {
      if (allowed) toast('Contacts enabled for this prototype.');
      gateHost.innerHTML = '';
    });
