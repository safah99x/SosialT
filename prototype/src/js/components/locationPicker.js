 ...
 *
 * Why custom (not Mapbox / Google Maps embed):
 * - The prototype is a closed device frame; we want zero third-party iframes,
 *   no system blue, no cookie banners, no pinch-to-zoom hijacks.
 * - The brief asked for a *clean, beautifully aligned-with-the-brand*
 *   experience. We render a tonal, illustrative "map" using SVG paths, in
 *   the warm cream / sage / taupe palette of the Atmospheric Hygge system.
 *
 * Layout:
 *   [ Search field ]
 *   [ Quick chips: Use current . At my place . Drop a pin ]
 *   [ Stylised map preview with centre pin ]
 *   [ Section: Recent / Suggested places ]
 *   [ "Set as location" CTA ]
 *
 * Usage:
 *   mountLocationPicker({ initial, onPick: (place) => {...} })
 *   place = { name, sub, kind }
 */
...
/**
 * Tonal SVG "map" art. Not a real geo-projection — it's an illustrative,
 * brand-aligned cartography. Roads, parks, water, blocks. The centre pin
 * sits in the middle and rests on a gentle ripple.
 */
function mapArt() {
  return `
...
export function createLocationPicker({ initial, onPick } = {}) {
  ...
  search.addEventListener('input', () => {
    ...
    clear.hidden = !q;
    list.querySelectorAll('.loc-row').forEach((r) => {
      const name = r.querySelector('.loc-row__name').textContent.toLowerCase();
      const sub  = r.querySelector('.loc-row__sub').textContent.toLowerCase();
      r.style.display = !q || name.includes(q) || sub.includes(q) ? '' : 'none';
    });
  });
